...
class Trainer:
    ...
    def train_step(self):
        ...
        input_photo_scenery, len_photo_scenery = DataLoader(self.photo_scenery_path, image_shape=self.image_shape,
                                                            batch_size=self.batch_size).run()
        input_cartoon_scenery, len_cartoon_scenery = DataLoader(self.cartoon_scenery_path, image_shape=self.image_shape,
                                                                batch_size=self.batch_size).run()

        min_len = min(len_photo_face, len_cartoon_face, len_photo_scenery, len_cartoon_scenery)

        # Start training
        for epoch in range(self.epochs + 1):
            g_loss = 0
            d_loss = 0
            pbar = tqdm(range(min_len // self.batch_size + 1))
            ...
            g_loss_total = g_loss / self.batch_size
            d_loss_total = d_loss / self.batch_size
